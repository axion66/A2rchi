...
from src.a2rchi.models.byok_factory import byok_context
# from src.data_manager.data_manager import DataManager
from src.data_manager.data_viewer_service import DataViewerService
from src.utils.yaml_config import CONFIGS_PATH, list_config_names, load_yaml_config
from src.utils.env import read_secret
from src.utils.logging import get_logger
from src.utils.sql import (
    SQL_INSERT_CONVO, SQL_INSERT_CONVO_V2, SQL_INSERT_FEEDBACK, SQL_INSERT_TIMING, SQL_QUERY_CONVO,
    ...
    SQL_INSERT_AB_COMPARISON, SQL_INSERT_AB_COMPARISON_V2, SQL_UPDATE_AB_PREFERENCE, SQL_GET_AB_COMPARISON,
...
class ChatWrapper:
    ...
    def __init__(self):
        ...
        # store postgres connection info (needed before data viewer init)
        self.pg_config = {
        ...
        # BYOK user service (lazy-initialized)
    ...
    def user_service(self):
        """
        Lazy-initialized UserService for BYOK API key lookup.
        
        Returns None if UserService is not available (graceful fallback).
        """
        if self._user_service is None:
            try:
                from src.utils.user_service import UserService
                self._user_service = UserService(self.pg_config)
            except Exception as e:
                logger.warning(f"Could not initialize UserService for BYOK: {e}")
                self._user_service = False  # Mark as unavailable
        return self._user_service if self._user_service is not False else None

    def _ensure_user_exists(self, client_id: str) -> None:
        """
        Ensure user exists in users table, creating if needed.
        
        Implements spec requirement: "User creation on first interaction"
        ...
        For anonymous users (identified by client session/cookie):
        - auth_provider is set to 'anonymous'
        """
        if not self.user_service:
            return  # UserService not available, graceful fallback
        
        try:
            # Determine auth provider from client_id pattern
            # Anonymous users typically have session-generated IDs
            auth_provider = "anonymous"
            if client_id.startswith("sso_"):
                auth_provider = "sso"
            elif client_id.startswith("basic_"):
                auth_provider = "basic"
            
            # get_or_create_user handles the upsert logic
            ...
            logger.warning(f"Failed to ensure user exists for {client_id}: {e}")
    ...
    def _get_config_payload(self, config_name):
        if config_name not in self._config_cache:
            self._config_cache[config_name] = load_yaml_config(name=config_name)
    ...
    def create_ab_comparison(
        ...
        response_a_mid: int,
        response_b_mid: int,
        config_a_id: int = None,
        config_b_id: int = None,
        ...
        Supports both legacy (config_id) and V2 (model/pipeline) tracking.
        If model_a/model_b are provided, uses V2 schema.
        ...
        Args:
            ...
            config_a_id: (Legacy) Config ID used for response A
            config_b_id: (Legacy) Config ID used for response B
            is_config_a_first: True if config A was the "first" config before randomization
            model_a: (V2) Model identifier for response A
            pipeline_a: (V2) Pipeline identifier for response A
            model_b: (V2) Model identifier for response B
            pipeline_b: (V2) Pipeline identifier for response B
        ...
        cursor = conn.cursor()
        try:
            # Use V2 schema if model info is provided
            if model_a is not None or model_b is not None:
                cursor.execute(
                    ...
                    (conversation_id, user_prompt_mid, response_a_mid, response_b_mid,
                     model_a or 'unknown', pipeline_a or 'unknown',
                     model_b or 'unknown', pipeline_b or 'unknown',
                     is_config_a_first)
                )
            else:
                # Legacy: use config_id
                cursor.execute(
    ...
    def insert_conversation(
        ...
        """
        Insert conversation messages into the database.
        
        If model_used/pipeline_used are provided, uses the V2 schema (model tracking).
        Otherwise falls back to legacy conf_id-based insertion.
        """
        ...
        # Use V2 schema if model tracking info is provided
        use_v2 = model_used is not None or pipeline_used is not None
        
        if use_v2:
            # V2: Use model_used/pipeline_used columns instead of conf_id
            insert_tups = (
                [
                    # (service, conversation_id, sender, content, link, context, ts, model_used, pipeline_used)
                    (service, conversation_id, user_sender, user_content, '', '', user_msg_ts, None, None),
                    (service, conversation_id, a2rchi_sender, a2rchi_content, link, a2rchi_context, a2rchi_msg_ts, model_used, pipeline_used),
                ]
                if not is_refresh
                else [
                    (service, conversation_id, a2rchi_sender, a2rchi_content, link, a2rchi_context, a2rchi_msg_ts, model_used, pipeline_used),
            ...
            sql_query = SQL_INSERT_CONVO_V2
        else:
            # Legacy: Use conf_id foreign key
            insert_tups = (
                [
                    # (service, conversation_id, sender, content, link, context, ts, conf_id)
            ...
            sql_query = SQL_INSERT_CONVO

        # create connection to database (use local vars for thread safety)
        conn = psycopg2.connect(**self.pg_config)
        cursor = conn.cursor()
        psycopg2.extras.execute_values(cursor, sql_query, insert_tups)
        conn.commit()
    ...
    def _prepare_chat_context(
        ...
        # Auto-create user on first interaction (per spec: Users Table requirement)
        self._ensure_user_exists(client_id)
    ...
    def _finalize_result(
        ...
        *,
        ...
        # Extract model tracking info from pipeline result metadata
        metadata = result.get("metadata", {})
        model_used = metadata.get("model_used")
        pipeline_used = metadata.get("pipeline_used")
    ...
    def __call__(self, message: List[str], conversation_id: int|None, client_id: str, is_refresh: bool, server_received_msg_ts: datetime,  client_sent_msg_ts: float, client_timeout: float, config_name: str):
        ...
        Supports BYOK (Bring Your Own Key) - if the user (identified by client_id)
        has stored their own API keys, those will be used for LLM calls.
        """
        ...
        try:
            ...
            # Use BYOK context if user service is available
            # This allows users to use their own API keys stored in PostgreSQL
            with byok_context(user_id=client_id, user_service=self.user_service):
    ...
    def stream(
        ...
        *,
        ...
        try:
            ...
            # Use BYOK context for streaming - enables user's own API keys
            # We manually enter/exit context since generators can't use 'with' across yields
            from src.a2rchi.models.byok_factory import set_current_user, clear_current_user
            set_current_user(client_id, self.user_service)
...
class FlaskAppWrapper(object):
    ...
    def __init__(self, app, **configs):
        ...
        # Register V2 API Blueprint (PostgreSQL-consolidated services)
        if V2_API_AVAILABLE:
            try:
                register_v2_api(self.app)
                logger.info("V2 API registered at /api/v2/*")
            except Exception as e:
                logger.warning(f"Failed to register V2 API: {e}")
        else:
            logger.info("V2 API not available (missing dependencies)")
    ...
    def get_configs(self):
        ...
        for name in config_names:
            ...
            config_id = None
            try:
                payload = load_yaml_config(name=name)
