#!/bin/bash
# Run the chat app locally against the integration test database
#
# Prerequisites:
...
# Ensure retriever defaults exist for hybrid search
python -c "
import yaml
...
data_manager = config.setdefault('data_manager', {})
retrievers = data_manager.setdefault('retrievers', {})
hybrid = retrievers.setdefault('hybrid_retriever', {})

hybrid.setdefault('num_documents_to_retrieve', 5)
hybrid.setdefault('bm25_weight', 0.3)
hybrid.setdefault('semantic_weight', 0.7)
...
if [ -n "${OPENAI_API_KEY:-}" ]; then
    ...
    echo "OPENAI_API_KEY not set - using local Ollama only"
fi
...
with conn.cursor() as cur:
    ...
    ''', (
        ...
        psycopg2.extras.Json(data_manager.get('sources', {})),
    ))

conn.commit()
conn.close()
print('Config stored in DB')
"

# Start the chat service
python -m src.bin.service_chat
